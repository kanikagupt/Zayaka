...
function AddRecipe() {
    ...
    const addRecipe =() => {
        if(file===null || recipename==='' || time===0 || description==='' || ingredients.length===0 || instructions.length===0 || category.length===0){
        ...
        service.uploadImage(file).then(response=>{
            ...
            service.createPost({recipename,time,description,ingredients,instructions,category,featuredImages:response.$id,userid:id})
            .then(res=>{
                setCategory([]);
                setDescription('');
                setIngredients([]);
  ...
  return (
    <div  className='w-screen h-full'>
        <h1 className='h-text px-8 mt-10 text-[2rem]'>Add Recipe</h1>
        <div className='w-full h-fit flex flex-col justify-center items-center'>
        <div className='w-full h-fit flex justify-around'>
            <Dropzone setFile={setFile} setPreview={setPreview} className=' w-full sm:w-5/12 rounded-xl h-56 flex justify-center items-center bg-slate-50 border-2 border-dashed mb-4' />
            <div className=" w-full sm:w-1/2 mr-10 bg-slate-50 rounded-xl h-[50vh] flex flex-wrap justify-center py-4 upload-c">
            {file===null?<div className=' w-full h-full rounded-xl  flex justify-center items-center text-gray-300'>No Images Selected</div>:
            ...
            <div className='w-full sm:w-1/2 h-fit sm:px-10 py-12 flex justify-center items-center relative'>
                ...
                <div className=' w-full h-fit sm:mx-8 mx-5 px-4 flex flex-col z-10 rounded-lg shadow-[0_3px_10px_rgb(0,0,0,0.2)] bg-white'>
                    ...
                    <input className='sm:w-11/12 mx-4 h-10 border-2 border-orange-400 rounded-md resize-none px-2' type='text' placeholder='Enter Recipe Category' onKeyUp={addCategory}/>
                    {
                        ...
                        :<button onClick={addRecipe} className='sm:w-11/12 h-10 my-3 bg-orange-400 text-white rounded-md mx-4'>Add Recipe</button>
                    }
                </div>
            </div> 
